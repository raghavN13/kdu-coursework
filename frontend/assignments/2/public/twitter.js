...
const tweetBoxId = document.getElementById('tweet-box-id');
...
else if(username === 'sputnik_7'){
    personName = "Nikhil Bhalerao";
    personUsername = "sputnik_7";
    personUrl = "http://localhost:3000/michael.jpg";  
}

desktopImage.src = `${personUrl}`;
desktopName.innerText =  `${personName}`;
desktopUsername.innerText = `${personUsername}`;
mobileImage.src = `${personUrl}`;
mobileImageTwo.src = `${personUrl}`;  
mobileName.innerText = `${personName}` ;
mobileUsername.innerText = `${personUsername}`;
...
async function postTweet(username, tweetContent) {
    ...
    try {
        const response = await fetch('/users/feed', {
            method: 'POST',
            headers: {
...
async function fetchTweets(username, page) {
    try {
        const response = await fetch(`/users/feed?username=${username}&page=${page}&pageSize=${pageSize}`);
        if (response.ok) {
            const data = await response.json();
            return data;
        } else {
